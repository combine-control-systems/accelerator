...
from accelerator.blob import json_save, json_load, json_encode
...
def test(name, input, want_obj, want_bytes, **kw):
	...
	assert as_bytes == as_str.encode("utf-8"), "json_encode doesn't return the same data for as_str=True and False"
...
def synthesis():
	...
	try:
		...
		assert False, "Bytes are not supposed to work in json_encode"
	except TypeError:
		pass
