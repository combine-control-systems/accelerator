...
def test_numbers():
	...
	# Python 2 used to accept 42L as an integer, because the number
	# type falls back to python parsing. No longer expected to work.
...
def test_filter_bad_across_types():
	...
	def add_want(ix):
		v = data[ix]
		want.append((int(v[3]), v[1], json.loads(v[4]), v[6].decode('utf-8'), v[7],))
	for ix, v in enumerate(data):
		if v[0]:
			add_want(ix)
		dw.write(*v[1:])
